...
export const addUsersToChat = async (userLoggedInData, usersToAddData, chatData) => {
    ...
    await updateChatData(chatData.key, userLoggedInData.uid, { users: existingUsers.concat(newUsers) })
...
export const starMessage = async (messageId, chatId, userId) => {
    try {
        ...
        if (snapshot.exists()) {
            ...
            //star
            const starrredMessageData = {
                messageId,
                chatId,
                starredAt: new Date().toISOString(),
            }
            await set(childRef, starrredMessageData)
        }
    }
    catch (error) {
        console.log(error)
...
export const removeUserFromChat = async (userLoggedInData, userToRemoveData, chatData) => {
    ...
    const messageText = userLoggedInData.uid === userToRemoveData.uid ?
